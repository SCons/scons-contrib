...
def create_args_perl5_script(source, env):
    return (
        ['perl'] + get_perl_I(env) + [str(source[0])] + [str(a) for a in env['action']]
    )
...
def CheckPerlModule(context, module_name):
    ...
    is_ok = 0 == subprocess.call(
        ["perl", "-M" + module_name, "-e 1"], stdout=open(os.devnull, "wb")
    )
...
""",
    ...
    perl5_output_action = SCons.Script.Action(
        perl5_output, strfunction=perl5_output_strfunc
    )
    bld_perl5_output = SCons.Script.Builder(action=perl5_output_action)
    env.Append(BUILDERS={'PerlOutput': bld_perl5_output})

    perl5_script_action = SCons.Script.Action(
        perl5_script, strfunction=perl5_script_strfunc
    )
    bld_perl5_script = SCons.Script.Builder(action=perl5_script_action)
    env.Append(BUILDERS={'PerlScript': bld_perl5_script})

    perl5_sub_action = SCons.Script.Action(perl5_sub, strfunction=perl5_sub_strfunc)
