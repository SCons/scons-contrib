...
def _sipEmitter(targets, sources, env): 
	...
	for s in sources:
		if env.Execute(sbfCommand%(sbfFileName,str(s)), 'Generating sip dependencies for %s'%s) :
...
def generate(env): 
	...
	env['SIPFLAGS'] = flags
	env['SIPCOM']   = '$SIP $SIPFLAGS -s ${TARGET.suffix} -c ${SIPOUTPUTDIR and SIPOUTPUTDIR or "."} -c ${TARGET.dir} $SOURCES'
